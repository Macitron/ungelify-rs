...
impl MagesArchive {
        ...
        P: AsRef<Path>,
    {
        ...
        rpk_writer
            ...
            .unwrap();
        let mut rpk_entries = IndexMap::with_capacity(self.entries.len());
        for entry in self {
            ...
            let new_entry = if let Some(rpk_path) = rpk_paths.get(entry.name()) {
                ...
                entry.updated(new_entry_offset, src_len, bytes_written)
            } else {
                ...
                entry.updated(new_entry_offset, entry.len_deflated(), bytes_written)
            };

            rpk_entries.insert(entry.id(), new_entry);
        }
        ...
        rpk_writer
            ...
            .unwrap();
        for rpk_entry in rpk_entries.values() {
            if self.is_old_format {
                bytes::write_struct(rpk_writer, MpkEntryV1::from(rpk_entry));
            } else {
                bytes::write_struct(rpk_writer, MpkEntryV2::from(rpk_entry));
            }
        }
